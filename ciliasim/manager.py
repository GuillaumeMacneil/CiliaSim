...
class LoadedTissue:
    def __init__(
        ...
        self.tissue = Tissue(
            parameters["x"], parameters["y"], parameters["cilia_density"]
        )
    ...
    def load_iteration(self, iteration: int):
        if self.cell_states[str(iteration)]:
            ...
            self.tissue.adjacency_matrix = np.zeros(
                (self.tissue.num_cells, self.tissue.num_cells)
            )
            ...
            for cell in boundary_cells:
                ...
                differences = (
                    self.tissue.cell_points[boundary_neighbours]
                    - self.tissue.cell_points[cell]
                )
            ...
            if closest == None:
                ...
                for key in self.force_states[closest].keys():
                    ...
                    if int_key == -1:
                        ...
                        self.tissue.add_cilia_force(
                            int_key, self.force_states[closest][key]
                        )

            self.current_it = iteration
        else:
            raise ValueError(
                f"Iteration {iteration} does not lie in the range [{self.min_it}, {self.max_it}]"
            )
    ...
    def plot_tissue(
        ...
        information = (
            f"Iteration: {self.current_it}\nEx. Flow Force: {self.tissue.flow_force}"
        )
        plot_tissue(
            self.tissue.cell_points,
            self.tissue.cell_types,
            title,
            duration,
            self.tissue.plot,
            x_lim=x_lim,
    ...
    def plot_shape_factor_histogram(
        self, title: str, duration: float, auto: bool = True
    ):
        information = (
            f"Iteration: {self.current_it}\nEx. Flow Force: {self.tissue.flow_force}"
        )
    ...
    def interactive_tissue(
        self, title: str, start_iteration: int = 0, end_iteration: int = 0
    ):
        ...
        avg_major_axes_button_axis = self.tissue.plot.fig.add_axes(
            [0.65, 0.1, 0.09, 0.03]
        )
        ...
        shape_factor_button_axis = self.tissue.plot.fig.add_axes(
            [0.25, 0.05, 0.09, 0.03]
        )
...
class Manager:
    ...
    def interactive_plot(
        self, index: int, title: str, start_iteration: int = 0, end_iteration: int = 0
    ):
    ...
    def animate_plot(
        ...
        self.tissues[index].animate_tissue(
            title, plot_type, start_iteration, end_iteration, step
        )
    ...
    def batch_load_iteration(self, iteration: int):
        for tissue in self.tissues:
            if not (tissue.min_it <= iteration <= tissue.max_it):
                raise ValueError(
                    f"Batch load iteration must be valid for all tissues ({tissue.min_it} <= x <= {tissue.max_it})"
                )
