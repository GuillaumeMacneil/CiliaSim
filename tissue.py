...
class Tissue():
    ...
    def calculate_force_matrix(self):
        ...
        for i in range(self.num_cells):
            neighbours = neighbour_vertices[1][neighbour_vertices[0][i]:neighbour_vertices[0][i+1]]
            self.adjacency_matrix[i] = np.zeros(self.num_cells)
            self.adjacency_matrix[i, neighbours] = 1
        ...
        force_matrix = np.zeros((self.num_cells, self.num_cells, 2))
        ...
        for i in range(self.num_cells):
            ...
            force_matrix[i, neighbours] += force_magnitudes * unit_vectors
            ...
            if self.cell_types[i] != 1:
                ...
                force_matrix[i, neighbours] += area_difference * unit_vectors
                force_matrix[neighbours, i] += area_difference * unit_vectors

            if self.cell_types[i] == 2:
                ...
                force_matrix[i, neighbours] += external_forces 
    ...
    def evaluate_boundary(self):
        ...
        for i in boundary_cells[0]:
            for neighbour in np.where(self.adjacency_matrix[i] == 1):
                if neighbour in boundary_cells and neighbour not in visited:
        ...
        for a, b in edges:
            shared_cells = np.intersect1d(self.adjacency_matrix[a], self.adjacency_matrix[b])
            shared_non_boundary = np.setdiff1d(shared_cells, boundary_cells)
            if len(shared_non_boundary) == 0:
                continue
            c = shared_non_boundary[0]
            ...
            if angle > np.pi / 2:
                ...
                self.cell_points = np.append(self.cell_points, reflected_point)
    ...
    def set_cell_inits(self):
        # FIXME: All quantities should be in separate arrays for easier indexing
        for i in range(self.num_cells):
            self.cell_inits.append([self.cell_types[i], self.target_areas[i]])
